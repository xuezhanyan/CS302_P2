...
// Title:            Sticks
// Files:            Sticks.java, TestSticks.java, Config.java
 ...
 * loses the game (cited from the class page).
 * 
 * @author Xuezhan Yan
 */

public class Sticks {
	// pass constants from Config class that will be used in different methods
	...
	public static void main(String[] args) {
		...
		initialStickNum = promptUserForNumber(input,
				"How many sticks are there on the table initially ("
						+ MIN_STICKS + "-" + MAX_STICKS + ")?",
				MIN_STICKS, MAX_STICKS);
		// menu
		System.out.println("");
		...
		int choose = promptUserForNumber(input, "Which do you choose (1,2,3)?",
				1, 3);
		...
		case 1: {
			...
			System.out.println("");
			System.out.println(
					"There are " + initialStickNum + " sticks on the board.");
			int remainStick = 0;
		...
		case 3: {
			System.out.println("");
			// get number of ai training
			int numberOfGamesToPlay = 0;
			numberOfGamesToPlay = promptUserForNumber(
					input, "How many games should the AI learn from ("
							+ MIN_GAMES + " to " + MAX_GAMES + ")?",
					MIN_GAMES, MAX_GAMES);

			int[][] strategyTable = trainAi(initialStickNum,
					numberOfGamesToPlay);
			playAgainstComputer(input, initialStickNum, userName,
					strategyTable);
			// ask user whether print out strategy table
			char answer = ' ';
			do {
				System.out.print(
	...
	static int promptUserForNumber(Scanner input, String prompt, int min,
		...
		do {
			...
			if (input.hasNextInt()) {
				// input is a number but only leave loop when in the range
	...
	static void playAgainstFriend(Scanner input, int startSticks,
		...
		while (true) {
			if (runTimes % 2 == 0) {
				// action of player 2
				takenStick = promptUserForNumber(input,
						player2Name + ": How many sticks do you take ("
								+ MIN_ACTION + "-"
								+ Math.min(MAX_ACTION, startSticks) + ")?",
						MIN_ACTION, Math.min(MAX_ACTION, startSticks));
			} else {
				// action of player 1
				takenStick = promptUserForNumber(input,
			...
			// whether game end
			if (startSticks == 0) {
				// decides who wins depending on play times
				if (runTimes % 2 == 0) {
					System.out.println(
	...
	static int basicChooseAction(int sticksRemaining) {
		...
		if (sticksRemaining < MAX_ACTION && sticksRemaining > 0)
			...
			// randomly choose a number between MIN_ACTION and MAX_ACTION
			pick = Config.RNG.nextInt(MAX_ACTION - MIN_ACTION + 1) + MIN_ACTION;
	...
	static void playAgainstComputer(Scanner input, int startSticks,
		...
		while (true) {
			if (runTimes % 2 == 1) {
				...
				do {
					if (strategyTable == null)
						...
						takenStick = aiChooseAction(startSticks,
								strategyTable[startSticks - 1]);
					if (takenStick == 1)
						System.out.println(
								"Computer selects " + takenStick + " stick.");
					else
						System.out.println(
								"Computer selects " + takenStick + " sticks.");
				} while (takenStick < MIN_ACTION
						|| takenStick > Math.min(MAX_ACTION, startSticks));
			} else {
				// action of player
				takenStick = promptUserForNumber(input,
			...
			if (startSticks == 0) {
				...
				if (runTimes % 2 == 0) {
					System.out.println(
							playerName + " wins. " + "Computer" + " loses.");
					break;
				} else {
					System.out.println(
							"Computer" + " wins. " + playerName + " loses.");
					break;
				}
			} else if (startSticks == 1)
				System.out.println(
	...
	static int aiChooseAction(int sticksRemaining, int[] actionRanking) {
		...
		if ((actionRanking == null) || (sticksRemaining <= 0)
				|| (actionRanking.length == 0))
			returnValue = 0;
		else if (sticksRemaining < MAX_ACTION)
			...
			// generate a random number from 1 to total probability
			// representing probability of specific action
			int randNum = Config.RNG.nextInt(totalPossibility) + 1;
			// currently max range of probability
			int maxRange = actionRanking[0];
			// check whether the random number is in this current range
			for (int i = 0; i < actionRanking.length; i++) {
				if (randNum <= maxRange) {
					// pick corresponding sticks if random number is
					// in current range
					returnValue = MIN_ACTION + i;
					break;
				} else
					// increase the range to represent next action
					maxRange = maxRange + actionRanking[i + 1];
			}
		}
		return returnValue;
	}
	...
	static String actionRankingToString(int sticksLeft, int[] actionRanking) {
		...
		for (int i = 0; i < actionRanking.length; i++) {
			...
			// if this is last element, end with comma
			// else end with line separator
	...
	static void updateActionRankingOnLoss(int[] actionRanking, int action) {
		actionRanking[action - MIN_ACTION] = Math.max(1,
				actionRanking[action - MIN_ACTION] - 1);
	}
	...
	static void updateActionRankingOnWin(int[] actionRanking, int action) {
		actionRanking[action - MIN_ACTION] = actionRanking[action - MIN_ACTION]
				+ 1;
	}
	...
	static String strategyTableToString(int[][] strategyTable) {
		...
		for (int i = strategyTable.length - 1; i >= 0; i--) {
			returnString = returnString
					+ actionRankingToString((i + 1), strategyTable[i]);
		}
	...
	static int playAiVsAi(int startSticks, int[][] player1StrategyTable,
		...
		while (true) {
			if (runTimes % 2 == 1) {
				// get action of player2
				takenStickP2 = aiChooseAction(startSticks,
						player2StrategyTable[startSticks - 1]);
				// store action into player2ActionHistory
				player2ActionHistory[startSticks - 1] = takenStickP2;
				// update remaining sticks
				startSticks = startSticks - takenStickP2;
			} else {
				// get action of player1
				takenStickP1 = aiChooseAction(startSticks,
						player1StrategyTable[startSticks - 1]);
				// store action into player1ActionHistory
				player1ActionHistory[startSticks - 1] = takenStickP1;
				// update remaining sticks
	...
	static int[][] trainAi(int startSticks, int numberOfGamesToPlay) {
		// check special cases and return null
		if ((startSticks < MIN_STICKS) || (startSticks > MAX_STICKS)
		...
		// create number of wins integer for both players
		int numP1Win = 0;
		int numP2Win = 0;
		for (int i = 0; i < numberOfGamesToPlay; i++) {
			if (playAiVsAi(startSticks, player1StrategyTable,
					player1ActionHistory, player2StrategyTable,
					player2ActionHistory) == 1) {
				//player 1 win
		...
		//return better strategy depending on who win more games
